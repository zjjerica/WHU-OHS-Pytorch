...
def main():
    print('Build model ...')
    model_name = '1DCNN'
    model = CNN_1D(input_features=32, n_classes=24).to(device)
    ...
    train_dataset = WHU_OHS_Dataset(image_file_list=train_image_list, label_file_list=train_label_list, use_3D_input=True)
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=1 / math.e, patience=3,
